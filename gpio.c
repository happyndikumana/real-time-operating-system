...
#define OFS_DATA_TO_DIR    1*4*8 //1 = register traversed from the DATA register, each register is 4 bytes and each byte is 8 bits. So essentially 32 bits are traversed by register
...
{
    uint32_t* p;
    p = (uint32_t*)port + pin; //pointer math: Adds pin * 4 cuz every register is 32 bits 4 bytes
    *p = value;
}
